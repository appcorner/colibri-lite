...
fn representative_trace_capture() {
    ...
    for step in 0..processed_steps {
        #[cfg(feature = "m5-3-compute-profiling")]
        crate::profiling::set_phase(if step < input_token_ids.len() {
            "prefill".to_owned()
        } else {
            format!("decode_{}", step - input_token_ids.len() + 1)
        });
