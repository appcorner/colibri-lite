...
fn open_readers() -> HashMap<usize, R2_2LayerCandidateReader> {
    ...
    let group32_layout = R1_1PackedArtifactLayout::canonical(32).expect("D4D4 group32 layout");
    let group8_layout = R1_1PackedArtifactLayout::canonical(8).expect("D4D4 group8 layout");
    ...
    .expect("D4D4 frozen Layer0 group32 artifact");
    let layer24 = R2_2LayerCandidateReader::open(
        24,
        &d2_root.join("layer24-group8.bin"),
        group8_layout,
        "4d51c17a2ba040229a8c20c895c5be8f030ab273db3151ccb739df7289965435",
    )
    .expect("D4D4 frozen Layer24 group8 artifact");
...
fn packed_layer_candidate(
    ...
    match layer {
        ...
        .expect("D4D4 Layer24 hybrid candidate"),
        _ => panic!("D4D4 unexpected packed layer {layer}"),
    }
}
...
fn run_fixture(
    ...
    let config = PINNED_QWEN3_30B_A3B_CONFIG
        .map_to_f32_runtime()
        .expect("D4D4 runtime config")
        .runtime_config();
    let expert_layout = PackedExpertLayout::for_config(config);
    let mut payload =
        File::open(artifact_root.join(&plan.payload)).expect("open D4D4 dense payload");
    ...
    let mut cache = KvCache::new(48, processed_positions, 4, 128).expect("D4D4 fixed KV cache");
    ...
    for position in 0..processed_positions {
        let token_id = *sequence
            .get(position)
            .expect("D4D4 generated token available before position");
        assert_eq!(cache.len(), position, "D4D4 cache position");
        ...
        for layer in 0..48 {
            ...
            let attention = cached_attention_with_weights(
                ...
                cache.layer(layer).expect("D4D4 KV layer"),
            )
            .expect("D4D4 attention");
            let residual = elementwise_add(hidden.view(), attention.output.view())
                .expect("D4D4 attention residual");
            let post_norm = rms_norm(
                residual.view(),
                weights.post_norm.view(),
                config.rms_norm_epsilon(),
            )
            .expect("D4D4 post-attention norm");
            let router =
                route_tokens(post_norm.view(), weights.router.view(), config).expect("D4D4 router");
            ...
            let moe = if PACKED_LAYERS.contains(&layer) {
                ...
                .expect("D4D4 canonical F32 experts")
            };
            hidden = elementwise_add(residual.view(), moe.view()).expect("D4D4 block output");
        ...
        let update_views = updates
            ...
            .collect::<Vec<_>>();
        cache.append_token(&update_views).expect("D4D4 KV append");
        ...
        if position + 1 >= fixture.token_ids.len() {
            ...
            let greedy = greedy_token(logits.view()).expect("D4D4 finite greedy logits");
            if position + 1 == fixture.token_ids.len() {
    ...
    D4D4Run {
        prompt_top20_ids: prompt_top20_ids.expect("D4D4 prompt top20"),
        prompt_argmax: prompt_argmax.expect("D4D4 prompt argmax"),
        prompt_guard_ids,
        prompt_logits: prompt_logits.expect("D4D4 prompt logits"),
        prompt_logits_sha256: prompt_logits_sha256.expect("D4D4 prompt logits hash"),
        prompt_final_norm_sha256: prompt_final_norm_sha256.expect("D4D4 prompt norm hash"),
        prompt_fixed_logit_error: prompt_fixed_logit_error.expect("D4D4 fixed-logit error"),
        prompt_top20_logit_error: prompt_top20_logit_error.expect("D4D4 top20-logit error"),
        generated_ids,
...
fn assert_frozen_semantics(fixture: &TierBReference, frozen: &R1_2FrozenReference, run: &D4D4Run) {
    ...
    assert!(
        ...
        "{} D4D4 compact-logit error {observed_f32} exceeds {allowed_f32}",
        fixture.name,
    );
    assert!(run.prompt_logits.iter().all(|value| value.is_finite()));
}

fn assert_scalar_local(run: &D4D4Run, fixture: &str) {
...
fn assert_native_local(run: &D4D4Run, fixture: &str) {
...
#[test]
fn m6_3_r2_2_d4d4_group8_hybrid_pass_held_out_quality() {
    assert!(
        crate::r2_native::avx2_fma_available(),
        "D4D4 host must expose AVX2+FMA"
    );
    let reference_path = PathBuf::from(
        env::var_os("COLIBRI_R1_2_REFERENCE_PATH").expect("R1.2 frozen reference path"),
    );
    let output_path =
        PathBuf::from(env::var_os("COLIBRI_R2_2_D4D4_OUTPUT").expect("D4D4 quality output path"));
    assert!(!output_path.exists(), "D4D4 quality output must be new");
        ...
        .filter(|fixture| matches!(fixture.name.as_str(), "short_english" | "short_thai"))
    {
        ...
        .expect("write D4D4 quality evidence");
    }

    fs::write(output_path, evidence).expect("write D4D4 quality evidence file");
}
