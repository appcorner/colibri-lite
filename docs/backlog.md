...
| Resident dense weights plus strict global LRU | BACKLOG | This is only a proposed post-closure candidate, not an approved implementation task. M5 cache evidence supports retaining strict global LRU, but resident-dense memory semantics and end-to-end value have not been measured. | Simulation and a measurement plan define dense-residency bytes, cache budget, working-set semantics, and expected end-to-end gain; a reviewed experiment then preserves frozen F32 evidence and configured limits while showing repeatable full-runtime improvement. |
