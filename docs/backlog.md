...
| --- | --- | --- | --- |
| CUDA backend | NEXT | M6 permits one GPU backend only when M6.1 measures a material benefit and M6.3 reviews its correctness, dependency, and provenance surface. | The M6.1 profile and M6.2 planner identify CUDA as the selected M6.3 vertical-slice backend. |
...
| Resident dense weights plus strict global LRU | BACKLOG | M5.4-01 simulation and the M5.4-02 24-row runtime matrix are complete for review. Correctness and budget gates passed, but timing is directional, physical I/O/page-cache behavior was not measured, and runtime value was not established. Keep the prototype measurement-only; no production/default follow-up is authorized. | A new separately reviewed experiment preserves frozen F32 evidence, explicit total-RAM accounting, the configured limits, and the reference reader while demonstrating repeatable end-to-end improvement with controlled timing, working-set, and I/O measurement semantics. |
| TurboFieldfare-inspired layer-slot streaming | BACKLOG | The external research reference supports per-layer expert slots, bounded explicit positional reads, and hit-first overlap. Those ideas are directionally consistent with the intended M6.4 design, but M6.3-06 is a NO-GO: no admissible quantized candidate, material end-to-end benefit, working-set peak, or physical per-process I/O evidence exists. They must not be used to bypass the M6.3 gate or to port Swift/Metal code into this Windows Rust runtime. | A newly reviewed M6.3 proposal first admits a Qwen3 candidate under defined numerical-quality, physical-I/O, and working-set gates; then a revised M6.4 plan may separately benchmark per-layer capacity, bounded Windows positional reads, and deterministic hit-first scheduling against `reference-f32-v1`. |
