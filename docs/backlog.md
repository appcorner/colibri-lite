...
| Read-only memory mapping | BACKLOG | The portable verified reader meets M2 correctness/residency goals at 129.367 MiB/s on the recorded workload; mapping does not remove current F32 decode/copy and would add a dependency or unsafe lifetime boundary without bottleneck evidence. | Artifact I/O is at least 20% of measured decode wall time, or a reviewed prototype reduces end-to-end decode by at least 10% or removes a measured payload copy while preserving correctness, Windows lifetime behavior, and the resident-memory budget. |
