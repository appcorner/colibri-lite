...
term, request/result IDs, and output path. It hashes both input profile files
and emits the versioned planner-result JSON document.
...
host or promote an unavailable GPU. `--compute-gflop-per-token` is required
