...
COLLECTOR_SHA256 = "b576b19ba4fc190362f3bd9b85d1bf639e7751c500e9f277d911b3800cba958c"
...
def collect_sample(run_dir: Path, fixture: str, cache_label: str, pair: int, order_index: int, mode: str) -> dict:
    ...
    require(capture.get("schema") == "m6.3-r2.2-d5c-memory-capture-v1", "sample capture schema mismatch")
    require(capture["exit_code"] == 0, "sample process failed")
    require(capture["samples"] >= 1, "sample has no memory observations")
    handshake = capture.get("handshake", {})
    require(handshake.get("ready_before_go") is True, "sample READY/GO handshake missing")
    require(handshake.get("memory_sampler_active") is True, "sample memory sampler missing")
...
def main() -> int:
    ...
    require(
        sha256(repo / "scripts" / "capture_m6_3_r2_2_d5c_memory.ps1") == COLLECTOR_SHA256,
        "D5c memory collector hash mismatch",
    )
    ...
    capture_script = repo / "scripts" / "capture_m6_3_r2_2_d5c_memory.ps1"
    ...
    final = {
        ...
        "transport_remediation": "ADR-0116",
