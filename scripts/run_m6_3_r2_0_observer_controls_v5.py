#!/usr/bin/env python3
"""Run final-binary preregistered interleaved M6.3-R2.0 observer controls."""
...
TEST_FILTER = "full_model_validation_tests::r2_benchmark_tests::m6_3_r2_0_observer_interleaved_pair_sample"
...
def validate_inputs(args: argparse.Namespace) -> tuple[dict, dict, str, str]:
    ...
    require(manifest["schema"] == "m6.3-r2.0-execution-manifest-v5", "execution schema")
    ...
    require(manifest["localization_harness_amendment_sha256"] == HARNESS_AMENDMENT_SHA256, "harness amendment binding")
    require(manifest["prior_observer_v4_controls_sha256"] == OBSERVER_V4_SHA256, "observer v4 provenance")
    ...
    require(manifest["observer_controls_seen"] is False, "final controls already seen at freeze")
...
def run_pair(
    ...
    require(sample["schema"] == "m6.3-r2.0-observer-pair-sample-v3", "pair schema")
    require(sample["fixture"] == fixture["fixture_id"], "pair fixture")
    require(sample["path"] == path, "pair path")
    require(tuple(sample["base_observer_order"]) == order, "pair observer order")
    ...
    require(sample["mini_pairs_per_process"] == 5, "mini-pair count declaration")
    require(sample["timed_iterations_per_mini_pair_per_state"] == 5, "mini-pair iterations")
    require(sample["total_timed_iterations_per_state"] == 25, "total timed iterations")
    mini_pairs = sample["mini_pairs"]
    require(isinstance(mini_pairs, list) and len(mini_pairs) == 5, "mini-pair coverage")
    mini_overheads: list[float] = []
    enabled_total = 0
    disabled_total = 0
    expected_output = sample["output_sha256"]
    for index, mini in enumerate(mini_pairs):
        expected_order = order if index % 2 == 0 else (order[1], order[0])
        require(mini["mini_pair"] == index + 1, "mini-pair ordinal")
        require(tuple(mini["observer_order"]) == expected_order, "mini-pair observer order")
        require(mini["timed_iterations_per_state"] == 5, "mini-pair timed iterations")
        enabled = mini["states"]["enabled"]
        disabled = mini["states"]["disabled"]
        require(enabled["output_sha256"] == disabled["output_sha256"] == expected_output, "mini-pair output identity")
        require(disabled["timed_total_nanos"] > 0, "mini-pair disabled timing")
        enabled_total += enabled["timed_total_nanos"]
        disabled_total += disabled["timed_total_nanos"]
        mini_overheads.append(
            100.0 * (enabled["timed_total_nanos"] - disabled["timed_total_nanos"])
            / disabled["timed_total_nanos"]
        )
    overhead = statistics.median(mini_overheads)
    return {
        ...
        "instrumented_output_sha256": expected_output,
        "uninstrumented_output_sha256": expected_output,
        "instrumented_total_nanos": enabled_total,
        "uninstrumented_total_nanos": disabled_total,
        "mini_pair_overhead_percent": mini_overheads,
...
def main() -> int:
    ...
    if args.validate_only:
        print("R2.0 interleaved observer-control preflight: PASS")
        return 0
    ...
    try:
        ...
        print("R2.0 interleaved observer controls: PASS (20 pairs/processes)")
        return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"R2.0 interleaved observer controls: FAIL: {error}", file=sys.stderr)
        raise SystemExit(1)
