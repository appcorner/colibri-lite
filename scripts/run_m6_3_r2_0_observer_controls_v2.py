#!/usr/bin/env python3
"""Run the single pre-registered M6.3-R2.0 observer-remediation control matrix."""
...
REMEDIATION_CONTRACT_SHA256 = "7512369048ff4385bae38274f8e272eade02433d680ae01db0ebb0f12a46e714"
PRIOR_FAILED_CONTROLS_SHA256 = "140c219c846af500cc99bd352c3d8b3215571048e3cd02271f414bf84d28d64c"
...
def validate_inputs(args: argparse.Namespace) -> tuple[dict, dict, str, str]:
    ...
    require(args.remediation_contract.is_file(), "remediation contract is missing")
    ...
    remediation_sha = sha256_file(args.remediation_contract)
    runner_sha = sha256_file(Path(__file__).resolve())
    manifest = read_json(args.execution_manifest)
    remediation = read_json(args.remediation_contract)
    fixtures = read_json(args.fixture_record)
    binary_sha = sha256_file(args.binary)
    require(manifest["schema"] == "m6.3-r2.0-execution-manifest-v2", "execution schema")
    require(manifest["contract_sha256"] == CONTRACT_SHA256, "execution contract SHA")
    require(manifest["remediation_contract_sha256"] == remediation_sha == REMEDIATION_CONTRACT_SHA256, "remediation contract binding")
    require(manifest["prior_failed_observer_controls_sha256"] == PRIOR_FAILED_CONTROLS_SHA256, "prior failed controls binding")
    ...
    require(manifest["observer_runner_sha256"] == runner_sha, "observer runner SHA binding")
    ...
    require(manifest["prior_observer_controls_seen"] is True, "prior observer controls provenance")
    require(manifest["observer_controls_seen_for_this_execution_manifest"] is False, "v2 observer controls already seen at freeze")
    require(manifest["candidate_localization_results_seen"] is False, "localization results already seen")
    require(remediation["observer_controls_seen"] is True, "remediation prior control provenance")
    require(remediation["candidate_localization_results_seen"] is False, "remediation localization boundary")
    require(remediation["authorizes_observer_rerun"] is True, "remediation does not authorize observer rerun")
    ...
    return manifest, fixtures, binary_sha, remediation_sha
...
def main() -> int:
    ...
    parser.add_argument("--remediation-contract", type=Path, required=True)
    ...
    manifest, fixture_doc, binary_sha, remediation_sha = validate_inputs(args)
    if args.validate_only:
        print("R2.0 observer-remediation preflight: PASS")
        return 0
    ...
    try:
        ...
        document = {
            "schema": "m6.3-r2.0-observer-controls-v2",
            "status": "passed",
            "contract_sha256": CONTRACT_SHA256,
            "remediation_contract_sha256": remediation_sha,
            "prior_failed_observer_controls_sha256": PRIOR_FAILED_CONTROLS_SHA256,
        ...
        print(f"R2.0 observer remediation: PASS ({len(controls)} pairs)")
        return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"R2.0 observer remediation: FAIL: {error}", file=sys.stderr)
        raise SystemExit(1)
